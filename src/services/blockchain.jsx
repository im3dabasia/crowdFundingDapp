...
const createProject = async ({
  ...
  try {
    ...
    console.log("Inside create project 1", contract);
...
const loadProjects = async () => {
  try {
    ...
    const contract = await getEtheriumContract()
    console.log("inside loadproject 1", contract);
    const projects = await contract.getProjects()
    console.log("inside loadproject 2");

    const stats = await contract.stats()

    console.log("inside loadproject 3", stats);

    setGlobalState('stats', structureStats(stats))
    console.log("inside loadproject 4");

    setGlobalState('projects', structuredProjects(projects))
    console.log("inside loadproject 5");

  } catch (error) {
    console.log("inside loadproject Error hhhh");
...
const getVoteStatus = async (projectID, userID) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    // const connectedAccount = getGlobalState('connectedAccount')
    const contract = await getEtheriumContract()

    tx = await contract.getVotingStatus(projectID, userID, { gasLimit: 100000 })
